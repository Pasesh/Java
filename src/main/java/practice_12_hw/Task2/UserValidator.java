...
public class UserValidator {
    ...
    private void validateAge(User user){
        int age = user.getAge();
        if(age < 18 || age > 100){
            throw new InvalidUserException("Возраст пользователя должен быть в диапазоне от 18 до 100 лет");
        }
    }
    ...
    private void validateEmail(User user){
        ...
        if(email==null || email.isBlank()){
            throw new InvalidUserException("Почта пользователя не должна быть пустой");
        }
        if(!email.matches("^[A-Za-z0-9+_.-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,}$")){
            throw new InvalidUserException("Некорректный формат почты");
        };
    }
}
